...
int main() {
    ...
    const detail::QuotePrices one_tick_tighter =
        detail::quote_prices(
    ...
    const detail::QuotePrices repaired_ask =
        detail::quote_prices(
    ...
    const detail::QuotePrices repaired_bid =
        detail::quote_prices(
    ...
    // Integer lookahead is strict at the activation boundary and capped by
    // the requested span.  These examples exercise no-headroom, exact-
    // equality, finite-headroom, and explicit-cap cases.
    ...
    std::cout << "distributed quote and fixed-clock semantics tests passed\n";
    return 0;
}
