...
void print_usage(const char* program) {
    std::cout
        ...
        << "Exact globally ordered MPI execution of the interacting multi-asset LOB.\n"
        << "Books are partitioned by BookId % MPI ranks; model output must match\n"
        << "the one-process sequential reference for every rank count.\n\n"
        << "  --duration-seconds N       simulated seconds (23400 for one day)\n"
        << "  --sample-interval-ms X    state-trace interval (default 1000 ms)\n"
        << "  --seed N                   rank-independent model seed\n"
        << "  --books N                  number of interacting LOBs\n"
        << "  --data-dir PATH            empirical distribution directory\n"
        << "  --hawkes-rates-file CSV    optional configured_mu calibration\n"
        << "  --book-config-file CSV     per-book ITCH inputs and basket definition\n"
        << "  --quote-interval-ms X      shared-maker refresh interval\n"
        << "  --quote-quantity N         quantity at the inside quote\n"
        << "  --quote-levels N           adjacent levels quoted per side\n"
        << "  --quote-growth N           size multiplier for deeper levels\n"
        ...
        << "  --enable-etf-arbitrage     enable QQQ/reduced-basket arbitrage\n"
        ...
        << "  --enable-value-agent        enable one value agent per book\n"
        ...
        << "  --value-fundamental-volatility-bps X latent volatility\n"
        << "  --value-interval-ms X       value decision interval\n"
        << "  --value-order-latency-ns N  positive value-order latency\n"
        << "  --output-dir PATH          rank-zero summary output directory\n"
        << "  --exposure-threshold X     shared-maker hedge threshold\n"
        << "  --enable-shared-mm-hedging activate cross-book maker hedges\n"
...
dlob::SequentialMultiAssetConfig parse_arguments(int argc, char** argv) {
    ...
    config.output_dir = "results/exact_mpi_multi_asset";
    ...
    for (int index = 1; index < argc; ++index) {
        ...
        if (argument == "--duration-seconds") {
            ...
            const double milliseconds = parse_double(
                require_value(index, argc, argv, argument.c_str()), argument.c_str());
            ...
            if (!std::isfinite(milliseconds) || milliseconds <= 0.0
                ...
                throw std::invalid_argument("invalid --sample-interval-ms");
            }
            ...
            if (!std::isfinite(milliseconds) || milliseconds <= 0.0
                || milliseconds > maximum_milliseconds) {
                throw std::invalid_argument("invalid --quote-interval-ms");
            }
            ...
            config.etf_arbitrage.trigger_bps = parse_double(
                require_value(index, argc, argv, argument.c_str()), argument.c_str());
        } else if (argument == "--arbitrage-release-bps") {
            config.etf_arbitrage.release_bps = parse_double(
                require_value(index, argc, argv, argument.c_str()), argument.c_str());
            ...
            if (!std::isfinite(milliseconds) || milliseconds <= 0.0
                || milliseconds > maximum_milliseconds) {
                throw std::invalid_argument("invalid --arbitrage-interval-ms");
            }
            ...
            config.fundamental_value.threshold_bps = parse_double(
                require_value(index, argc, argv, argument.c_str()), argument.c_str());
        } else if (argument == "--value-response-bps") {
            config.fundamental_value.response_step_bps = parse_double(
                require_value(index, argc, argv, argument.c_str()), argument.c_str());
            ...
            if (!std::isfinite(milliseconds) || milliseconds <= 0.0
                || milliseconds > maximum_milliseconds) {
                throw std::invalid_argument("invalid --value-interval-ms");
            }
            ...
            config.market_maker_exposure_threshold = parse_double(
                require_value(index, argc, argv, argument.c_str()), argument.c_str());
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        const dlob::ExactMpiMultiAssetResult execution = simulator.run();
        const dlob::SequentialMultiAssetResult& result = execution.model;
        if (rank == 0) {
            std::cout << std::setprecision(6) << std::fixed
                      << "exact_mpi_multi_asset"
                      << " ranks=" << execution.world_size
                      ...
                      << " trade_hash=0x" << std::hex
                      << result.combined_trade_hash << std::dec
            ...
            for (const dlob::MultiAssetBookSummary& book : result.books) {
                std::cout << "book=" << book.book_id
                          << " symbol=" << book.symbol
                          << " owner_rank="
                          << dlob::ExactMpiMultiAssetSimulator::owner_rank(
                                 book.book_id, execution.world_size)
        ...
        MPI_Finalize();
        return result.structurally_valid ? 0 : 3;
    } catch (const std::exception& error) {
        std::cerr << "exact_mpi_multi_asset rank " << rank << ": "
                  << error.what() << '\n';
