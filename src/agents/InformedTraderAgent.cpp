...
InformedTraderAgent::InformedTraderAgent(int owner_id,
...
void InformedTraderAgent::generate_orders(const MarketState& current,
    ...
    while (next_wake_ns_ < window_end_ns) {
        ...
        const double private_signal = current.fundamental_value_ticks
            + config_.signal_noise_ticks * config_.tick_size * rng_.normal01();
        const double mispricing_ticks = (private_signal - current.mid_price_ticks)
        ...
        if (std::abs(mispricing_ticks) > config_.trade_threshold_ticks) {
            ...
            if (risk_allowed) {
                ...
                builder.emit(out, AgentKind::Informed, owner_id_, OrderAction::Market,
                             side, quantity, 0, 0, decision_time, rng_);
            }
        }
