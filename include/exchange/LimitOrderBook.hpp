...
// Read-only value of a hypothetical terminal inventory liquidation. The
// preview walks displayed external liquidity without mutating the book.
struct TerminalLiquidationPreview {
    std::int64_t requested_quantity = 0;
...
public:
    explicit LimitOrderBook(int tick_size, BookId book_id = 0);
    ...
    MarketState state_excluding_owner(
        std::int64_t time_ns,
