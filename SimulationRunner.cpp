...
constexpr const char*
    FITTED_HAWKES_PARAMETERS_FILE =
        "hawkes_mle_fixed_beta_lbfgsb_eta010/fixed_beta_mle_params_flat.csv";

std::string trim_csv_cell(std::string value) {
    if (
        value.size() >= 3
...
struct FittedHawkesParameters {
    double beta = 10.0;
    HawkesProcess::Vector6 mu{};
    HawkesProcess::Matrix6 branching{};
    HawkesProcess::Matrix6 alpha{};
};

FittedHawkesParameters load_fitted_hawkes_parameters(
    ...
    if (!input.is_open()) {
        throw std::runtime_error(
            "Could not open fitted Hawkes parameter file: "
            + filename
        );
    }
    ...
    if (
        ...
        || !std::getline(input, value_line)
    ) {
        throw std::runtime_error(
            "Fitted Hawkes parameter file must contain "
            "one header row and one value row."
        );
    }
    ...
    const std::vector<std::string> values =
        split_csv_line(value_line);

    if (headers.size() != values.size()) {
        throw std::runtime_error(
            "Fitted Hawkes parameter CSV has mismatched "
            "header and value column counts."
        );
    }
    ...
    FittedHawkesParameters fitted;

    if (table.count("beta") > 0) {
        fitted.beta = table.at("beta");
    }
    ...
    for (int i = 0; i < NUM_EVENT_TYPES; ++i) {
        for (int j = 0; j < NUM_EVENT_TYPES; ++j) {
            const std::string n_key =
                "N_"
                + std::to_string(i)
                + std::to_string(j);

            fitted.branching[i][j] =
                table.at(n_key);
...
CalibrationResult run_single_simulation(
    ...
    const FittedHawkesParameters fitted_hawkes =
        load_fitted_hawkes_parameters(
            FITTED_HAWKES_PARAMETERS_FILE
        );
